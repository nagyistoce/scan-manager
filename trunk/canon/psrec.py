...
class SDK(object):
	...
	def __init__(self):
		...
		self.dll = windll.LoadLibrary(r'PRSDK.dll')
...
def readString(size,ptr):
	chars = cast(v,c_byte).value
	return (c_wchar * chars).from_address(ptr)
...
class Camera(object):
	...
	def __init__(self,sdk,deviceInfo):
		...
		self.name = unicode(deviceInfo.DeviceInternalName)
		self.model = unicode(deviceInfo.ModelName)
	...
	def pformat(self,short=False,indent=''):
		if short:
			...
			s += indent + '  events: %s\n'%(', '.join([EventCode[i] for i in self.eventCodes if i in EventCodes]))
	...
	def getProperties(self):
		...
		for propCode in self.propCodes:
			...
			property = CameraProperty(camera=self,propDesc=propDesc)
			self.properties[property.name or property.propCode] = property


	def updateSingleProperty(self,propCode):
		if isinstance(propCode,basestring):
	...
	def stopRemote(self,quick=False):
		""" Stop relase control mode (retract the lens etc.) """
	...
	def viewfinderCallback(self,cameraHandle,context,size,dataPointer):
		self.vfData = str(string_at(dataPointer,size))
